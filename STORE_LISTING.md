...
### X（旧Twitter）で、不快な画像や過激な画像が突然目に入ってしまった経験はありませんか？

X（旧Twitter）の利用者さま向けの無料のGoogle Chrome拡張機能です。

X（旧Twitter）のDMやタイムライン上の画像をAIで自動判定し、
不快な画像や過激な画像を「不適切な画像」としてブロック表示します。
ブロックされた画像は、必要な場合のみ個別に表示できます。
また、画像フィルターの感度は拡張機能のスライダーで調整可能です。

Google ChromeでX（旧Twitter）をご利用いただく方が、本拡張機能を導入することで、
XのDMやタイムライン上で不意に不快な画像や過激な画像が表示されることによる、
心理的負担を和らげます。
...
🛡️ **自動ブロック**
X(旧Twitter)のタイムライン、DMに不意に流れてくる不快な画像や過激な画像をリアルタイムで判定し、自動的にブロック表示します。
