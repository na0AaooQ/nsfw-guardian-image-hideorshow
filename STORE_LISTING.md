...
X(Twitter)のタイムライン・DMのセンシティブ画像をAIで自動検出しブロック。クリックで個別に表示でき、感度も調整可能。画像データは外部送信しません。
...
「強制的に遮断するのではなく、そっと寄り添って守る」をコンセプトに、あなたのペースで安心して X を使えるようサポートします。
...
🛡️ **自動ブロック**
タイムライン・DMに流れてくるセンシティブ画像をリアルタイムで判定し、自動的にブロック表示します。

👁️ **クリックで個別表示**
ブロックされた画像でも「クリックで表示」ボタンを押すことで、1枚ずつ自分で確認できます。
...
🔒 **完全ローカル処理**
画像データは外部サーバーへ一切送信しません。すべての AI 判定はあなたのブラウザ内で完結します。
